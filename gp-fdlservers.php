...
if (is_login() == false) {
    ...
    $proveri_servere = mysql_num_rows(mysql_query("SELECT * FROM `serveri` WHERE `user_id` = '$_SESSION[userid]' AND `igra` = '7'"));
    if (!$proveri_servere) {
                        ...
                        <div id="serveri">
                            <center><table class="darkTable">
                                <tbody>
                                    ...
                                    <?php  
                                        $gp_obv = mysql_query("SELECT * FROM `serveri` WHERE `user_id` = '$_SESSION[userid]' AND `igra` = '7'");
                                        ...
                                        while($row = mysql_fetch_array($gp_obv)) { 
											...
											$igra = "img/icon/gp/game/fdl.png";
											
											$server_ip = ipadresa($srw_id);
                                        ?>
                                        <tr>
                                            <td>
                                                <img src="<?php echo $igra; ?>" style="width: 15px;">
                                                <a href="gp-fdlinfo.php?id=<?php echo $srw_id; ?>"><?php echo $naziv_servera ?></a>
                                            </td>
                                            <td><?php echo $istice; ?></td>
                                            <td><?php echo $cena; ?> &euro;</td>
                                            <td class="ip"><?php echo $server_ip; ?></td>
