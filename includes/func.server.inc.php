...
function ssh_dodaj_server($ip, $port, $username, $password, $novi_user, $novi_user_pw, $mod)
{
	...
	else 
	{
		...
		else 
		{
			...
			$cmd_chown = 'chown -Rf '.$novi_user.':'.$novi_user.' /home/'.$novi_user.'/*';
...
function reinstall_server($ip, $port, $username, $password, $serverid, $klijentid)
{
	...
	else 
	{
		...
		else 
		{
			...
			$cmd_chown = 'chown -Rf '.$server["username"].':'.$server["username"].' /home/'.$server["username"].'/* && chmod -R 755 /home/'.$server["username"].'/*';
			$cmd_clean = 'rm -rf '.$mod["zipname"].' && rm -rf wget-log';
