...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_query_cached($type, $ip, $c_port, $q_port, $s_port, $request, $id = NULL)
  {
    ...
    $cache      = empty($mysql_row['cache'])      ? array()      : unserialize($mysql_row['cache']);
  ...
  function lgsl_url_path()
  {
    ...
    else
    {
      ...
      if(isset($_SERVER['PATH_TRANSLATED'])) 
	  {
		  ...
		  $base_path = str_replace("\\", "/", $base_path);
		  $base_path = str_replace("//", "/", $base_path);
		  if(isset($_SERVER['SCRIPT_NAME'])) $base_path = substr($base_path, 0, -strlen($file_path));
	  }
    }
