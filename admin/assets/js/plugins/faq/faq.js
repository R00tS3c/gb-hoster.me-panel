...
(function($) {
    ...
    $.fn.goFaq = function(options) {
		...
		function generateEmptySearch () {
			...
			return $empty.html ('Nema rezultata');
		}
        
        function generateSearchForm () {
        	
        	var $form = $('<form>', { 'class': 'faq-search' });
        	var $input = $('<input>', { 'type': 'text', 'name': 'search', 'placeholder': 'Pretraga odgovora' });
        ...
        function generateToc ($list) {
        	var html = '<div id="atiketi">';	
        	
			$list.find ('li').each (function (i) {				
				var id = parseInt (i+1);							
				html += '<li>' + id + '. <a href="#faq-' + id + '">' + $(this).find ('#poruka').text () + '</a></li>';								
				$(this).attr ('id', 'faq-' + id);				
			});	
