...
$naslov = "Pregled error logova";
$fajl = "kerror_log";

	
	$adjacents = 3;

	$query = "SELECT COUNT(*) as num FROM error_log WHERE `vrsta` = '2'";
	$total_stranas = mysql_fetch_array(mysql_query($query));
	$total_stranas = $total_stranas[num];
	
	$targetstrana = "kerror_log"; 	
	$limit = 15; 	
	...
	$sql = "SELECT * FROM `error_log` WHERE `vrsta` = '2' ORDER by `id` DESC LIMIT $start, $limit";
				...
				<div class="widget-content">
					...
					<?php	
							if(mysql_num_rows($result) == 0) {
								echo'<tr><td colspan="5"><m>Trenutno nema klijent error logova.</m></td></tr>';
							}
							while($row = mysql_fetch_array($result)) {	?>
							<tr>
								<td>#<?php echo $row['id']; ?></td>
								<td><?php echo $row['string']; ?></td>
								<td><m><?php echo $row['file']; ?></m></td>
								<td><?php echo 'Linija: <m>#'.$row['line'].'</m>'; ?></td>
					...
					?>
						...
						<form action="process.php" method="POST">
							<input type="hidden" name="task" value="izbrisi_errorlog" />
